...
public class AdminApiController : ControllerBase
{
    ...
    public async Task SetAlert([Required, FromBody] SetAlertRequest request)
    {
        ...
        if (request.url != null)
        {
            ...
            if (!url.Host.EndsWith(".neoblx.xyz") && url.Host != "neoblx.xyz")
                throw new StaffException("All URLs must link to neoblx.xyz. Base was " + url.Host);
        }
...
-The Roblox Team"
    ...
    public async Task UpdateAssetProduct([Required, FromBody] UpdateProductRequest request)
    {
        ...
        if (!StaffFilter.IsOwner(safeUserSession.userId))
        {
            ...
            if (safeUserSession.userId != 19 && !allowedTypes.Contains(extraInfo.assetType))
    ...
    private async Task CopyItemFloodCheck()
    {
        if (!StaffFilter.IsOwner(safeUserSession.userId))
        {
            var canUploadLocal = await services.cooldown.TryIncrementBucketCooldown(
                "CopyItemFromRobloxV1:" + safeUserSession.userId, 10, TimeSpan.FromHours(0));
            if (!canUploadLocal)
            ...
            var canUploadLocalDay = await services.cooldown.TryIncrementBucketCooldown(
                "CopyItemFromRobloxV1:" + safeUserSession.userId, 20, TimeSpan.FromHours(0));
