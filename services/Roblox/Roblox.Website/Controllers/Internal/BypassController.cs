...
namespace Roblox.Website.Controllers
{
	...
	[HttpGet("/Setting/QuietGet/RCCService")]
	public dynamic Nigger()
	{
		return new { };
	}
    [HttpGet("/login")]
	public dynamic DityKitty()
	{
        return Redirect("https://www.neoblx.xyz/auth/login");
	}
    [HttpGet("/discord")]
	public dynamic DIscord()
	{
        return Redirect("https://discord.gg/apmjRddFyb");
	}
        [HttpGet("/Games.aspx")]
	public dynamic GameIsGame()
	{
        return Redirect("https://www.neoblx.xyz/games");
	}
    ...
    [HttpGet("/Setting/QuietGet/ClientAppSettings")]
    public dynamic DirtyJew()
    {
        try
        {
            string jsonFilePath = Path.Combine(Environment.CurrentDirectory, "Flags", "nigger.json");
	...
	public dynamic Faggot()
	{
        List<string> HashList = new List<string> {
            "0.235.0pcplayer",
        };
        return new { data = HashList };
	}
    [HttpGet("/GetAllowedMD5Hashes")]
	public dynamic Retard()
	{
        List<string> HashList = new List<string> {
            "06402c5ecd42ded0776f2ef48ff5e20d",
        };
        return new { data = HashList };
	}
        ...
        public string ValidatePlaceJoin(long originPlaceId, long destinationPlaceId)
        {
            return "true";
        }
        ...
        public async Task<MVC.ActionResult> GetAssetById(long id)
        {
            ...
            switch (details.assetType)
            {
                ...
                case Models.Assets.Type.SolidModel:
                    if (latestVersion.contentUrl is null)
                        throw new RobloxException(400, 0, "BadRequest meow"); // todo: should we log this?
        ...
        public void NegotiateAuthentication(string suggest)
        {
            ...
            {
                Domain = Request.GetDisplayUrl().Contains("localhost") ? "localhost" : ".neoblx.xyz",
                Secure = false,
                Expires = null,
                IsEssential = true,
                Path = "/",
        ...
        public async Task<dynamic> PlaceLaunch(long placeId)
        {
            ...
            if (result.status == JoinStatus.Joining)
            {
                ...
                return new
                {
                    ...
                    joinScriptUrl = $"http://www.neoblx.xyz/Game/Join.ashx?jobId={result.job}&placeId={placeId}&token={Request.Cookies[".ROBLOSECURITY"]}",
                    authenticationUrl = "http://www.neoblx.xyz" + "/Login/Negotiate.ashx",
        ...
        public async Task<dynamic> JoinGame(string jobId, long placeId)
        {
            ...
            {
                ClientPort = 0,
                MachineAddress = "104.238.173.181",
        ...
        [HttpGetBypass("Asset/PackageFetch.ashx")]
        public async Task<string> PackageFetch(long userId)
        {
            var assets = await services.assets.GetPackageAssets(userId);

            return
                $"{string.Join(";", assets.Select(c => Configuration.BaseUrl + "/Asset/?id=" + c))}";
        }
        ...
        {
            return new MVC.RedirectResult("https://discord.gg/9dH6VeDaJW");
        }
    [HttpGet("/integration-test/create-account-and-set-cookie")]
    public MVC.IActionResult VulnAbusingCunt()
        {
            return new MVC.RedirectResult("https://www.neoblx.xyz/auth/vulnabuser");
        }
    [HttpGet("/auth/password-reset")]
    public MVC.IActionResult VulnAbusingCunt2()
        {
            return new MVC.RedirectResult("https://www.neoblx.xyz/auth/vulnabuser");
        }
        [HttpGet("/swagger/index.html")]
        public MVC.IActionResult VulnAbusingCunt3()
        {
            return new MVC.RedirectResult("https://www.neoblx.xyz/auth/vulnabuser");
        }
        [HttpGet("/swagger")]
        public MVC.IActionResult VulnAbusingCunt4()
        {
            return new MVC.RedirectResult("https://www.neoblx.xyz/auth/vulnabuser");
        }
...
{
    public string value { get; set; }  // Adjust the type of 'value' as needed
}
...
public async Task<dynamic> Set(long placeId, string key, string type, string scope, string target, int valueLength)
{
    ...
    if (value == null)
        throw new RobloxException(400, 0, "BadRequest: Invalid or missing data");
